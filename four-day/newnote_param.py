...
for i in range(1, r_num):
    ...
    # 验证
    if result == 'ok':
        # if driver.find_element(By.NAME,title): 表示title元素有且存在
        if driver.find_element(By.NAME,title) and driver.find_element(By.NAME,content):
            print('success')
        else:
            print('fail')
    elif title == '':
        elem1 = driver.find_element(By.ID, 'com.youdao.note:id/title').text
        elem2 = driver.find_element(By.ID, 'com.youdao.note:id/summary').text
        if elem1 == elem2:
            print('success')
        else:
            print('fail')
